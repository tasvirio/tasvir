...
class EntryStack {
 public:
  ...
  int PushNoLog(EntryIndex entry) {
    if (head_ >= size_) {
      return 0;
    }
...
class CuckooMap {
 public:
  ...
  bool IsReady() { return ready_.load(std::memory_order_acquire); }
  ...
  void Clear() {
    ...
    ClearInit();
    M()
    (this, Size(alloc_num_entries_,
                alloc_num_buckets_));  // This writes to the entire region. This
                                       // logs everrrryyyyyyything
  }
 ...
 protected:
  ...
  void ClearInit() {
    ...
    for (int i = alloc_num_entries_ - 1; i >= 0; --i) {
      free_entry_indices_->PushNoLog(i);
    }
    free_entry_indices_->InvalidateStack();
    ready_ = true;
    M()
    (this, sizeof(CuckooMap<K, V, H, E, M>));  // This writes to small thing, so
                                               // we don't have to log
                                               // everything.
  }

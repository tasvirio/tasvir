...
template <typename M = TasvirMarker>
class EntryStack {
 public:
...
template <typename K, typename V, typename H = std::hash<K>,
          typename E = std::equal_to<K>, typename M = TasvirMarker>
class CuckooMap {
 public:
  ...
  class iterator {
   public:
    ...
    iterator(const CuckooMap& map, size_t bucket, size_t slot)
  ...
  const iterator end() const { return iterator(*this, alloc_num_buckets_, 0); }
